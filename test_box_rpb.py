...
import psutil
import os
from functions import BoxPairRPBCUDAFunction, box_pair_rbp_python
...
def run_test():
    ...
    print(f"Testing with: B={B}, Ch={Ch}")
    ...
    mlp_weights = torch.randn(6*Ch+1, device=device, dtype=dtype)
    mlp_weights[:] = 0.0
    ...
    with profile(
        ...
        with record_function("pytorch_forward"):
            output_py = box_pair_rbp_python(mlp_weights_py, pos, pos, Ch)
    ...
    with profile(
        ...
        with record_function("cuda_forward"):
            output_cu = BoxPairRPBCUDAFunction.apply(mlp_weights_cu, pos, pos, Ch)
            torch.cuda.synchronize()
