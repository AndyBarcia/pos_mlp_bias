...
class PosGaussian(torch.nn.Module):
    """
    ...
    def _reset_parameters(self):
        ...
        offsets = self.weight_generator.bias[:self.n_heads*2]
        sigmas = self.weight_generator.bias[self.n_heads*2:]
        torch.nn.init.normal_(offsets, mean=0.0, std=0.1)
        torch.nn.init.normal_(sigmas, mean=1.0, std=0.1)

    def forward(
        self,
        pos: torch.Tensor, # (B,4) [x,y,w,h]
        size: Union[int, Tuple[int, int]], # (H,W) or int for square
        queries: torch.Tensor, # (...,C)
        implementation: str = "cuda"
    ) -> torch.Tensor: # (B,Nheads,H,W)
        pos, pos_ps = pack([pos], "* xywh")
        queries, _ = pack([queries], "* c")
        ...
        weights = self.weight_generator(queries) # (..., 2)
...
class PosGaussianAttention(torch.nn.Module):
    """
