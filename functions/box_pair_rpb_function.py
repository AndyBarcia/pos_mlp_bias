...
class BoxPairRPBCUDAFunction(Function):
    @staticmethod
    def forward(ctx, mlp_weights, pos1, pos2, c_hidden):
        mlp_weights = mlp_weights.contiguous().float()
        pos1 = pos1.contiguous().float()
        pos2 = pos2.contiguous().float()
        ctx.save_for_backward(mlp_weights, pos1, pos2)
        ctx.c_hidden = c_hidden
        output = pos_mlp_bias.forward_pair_rpb(mlp_weights, pos1, pos2, c_hidden)
        return output
    ...
    def backward(ctx, grad_output):
        ...
        grad_weights = pos_mlp_bias.backward_pair_rpb(grad_output, mlp_weights, pos1, pos2, ctx.c_hidden)
        return grad_weights, None, None, None


def box_pair_rbp_python(mlp_weights, boxes1, boxes2, c_hidden) -> torch.Tensor:
    """
    Python reference implementation for the box-pair RBP bias kernel.
    mlp_weights: ([4*C' + C' + 1*C' + 1])
    ...
    C = c_hidden
    assert mlp_weights.shape[0] == 6 * C + 1
    ...
    # Reshape MLP weights
    w1 = mlp_weights[:4 * C].view(4, C)  # (4, C)
    b1 = mlp_weights[4 * C:5 * C]        # (C,)
    w2 = mlp_weights[5 * C:6 * C].view(C, 1)  # (C, 1)
    b2 = mlp_weights[6 * C]              # scalar
    
    # Compute MLP output
    x = torch.einsum('bijk,kl->bijl', rel_features, w1) + b1  # (B1, B2, C)
    x = torch.softmax(x, dim=-1)
    x = torch.einsum('bijk,kl->bijl', x, w2) + b2  # (B1, B2, 1)
